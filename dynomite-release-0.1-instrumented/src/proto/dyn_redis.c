 ...
 * CS244B Updates Included in Lines 1637, 2066
 */ 
...
redis_parse_req(struct msg *r)
{
    ...
    log_debug(LOG_NOTICE," trace id %ld", msgID);
...
fragment:
    ...
    log_debug(LOG_NOTICE,"level = %d",r->level);
    ...
    char fname[128];
    sprintf(fname, "log_redis");
    FILE *f=fopen(fname,"a");
//    if (recvClientReq ==true) level=2;
//    else level =3;
   fprintf(f,"%ld %d %ld %s sent %d bytes\n",msgID,r->level, t,machine_id,r->pos-b->pos);
    fclose(f);
    num_sent_msgs=0;
    }
...
redis_parse_rsp(struct msg *r)
{
    ...
    sprintf(fname, "log_redis");
    FILE *f=fopen(fname,"a");
    fprintf(f,"%ld %d %ld %s received %d bytes\n",msgID,r->level, t,machine_id,r->pos-b->pos);
    fclose(f);
    num_recv_msgs=0;
    }
//    r->level=0;
