 ...
 * CS244B instrumentation added near line 49
 */ 
...
static uint32_t	num_rsp_msgs=0;
struct msg *
...
{
    struct msg *pmsg;
    log_debug(LOG_NOTICE,"got the response");
    ASSERT(!conn->dnode_client && !conn->dnode_server);
       char fname[128];
       sprintf(fname, "log_dynomite");
	num_rsp_msgs++;
       if (num_rsp_msgs == sampling_freq){
	...
	log_debug(LOG_NOTICE,"level %d, trace_id %ld", msg->level, msgID); 
      fprintf(f, "%ld %ld %ld %s recv %d bytes of ACK\n", msgID, msg->level, t, machine_id, msg->mlen);
       fclose(f);
	num_rsp_msgs=0;
      }
